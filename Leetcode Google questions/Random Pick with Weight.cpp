/*Explanation of why cumulative sum works:
...
public:
    ...
    int pickIndex() {
		...
		int scaledRandomNumber = rnd*totalWeight; //0-13
		int n = cumulativeSum.size();
		int l = 0, r = n-1;
		while(l <= r)
		{
			int mid = (l+r)/2;
			if(mid != 0 && scaledRandomNumber <= cumulativeSum[mid] && scaledRandomNumber > cumulativeSum[mid-1]) 
                return mid;
			else if(mid == 0 && scaledRandomNumber < cumulativeSum[mid]) return mid;
			else if(scaledRandomNumber > cumulativeSum[mid]) l = mid+1;
			else r = mid-1;
		}
		return n-1;
    }
